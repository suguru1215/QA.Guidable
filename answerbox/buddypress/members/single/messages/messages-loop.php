...
<?php if ( bp_has_message_threads( bp_ajax_querystring( 'messages' ) ) ) : ?>
	...
	<form action="<?php echo bp_loggedin_user_domain() . bp_get_messages_slug() . '/' . bp_current_action() ?>/bulk-manage/" method="post" id="messages-bulk-management">
		...
		<table id="message-threads" class="messages-notices">
			...
			<thead>
				<tr>
					<th scope="col" class="thread-checkbox"><label class="bp-screen-reader-text" for="select-all-messages"></label><input id="select-all-messages" type="checkbox"></th>
					<th scope="col" class="thread-from-avatar"></th>
						...
						<?php if ( 'sentbox' != bp_current_action() ) : ?>
							<td class="thread-from">
								<div class="message-user">
									...
									<?php $unread_count = bp_get_message_thread_unread_count( bp_get_message_thread_id() ); ?>
									<?php if($unread_count > 0): ?>
										<span class="message-unread"><?php printf(__('%d unread', 'ab'), $unread_count); ?></span>
									<?php endif; ?>
								</div>								
								<span class="message-date"><span class="from" title="<?php _e( 'Message received', 'ab' ); ?>"><i class="i-mail-forward"></i></span><?php bp_message_thread_last_post_date(); ?></span>
							</td>
						<?php else: ?>
							<td class="thread-from">								
								...
								<?php if($unread_count > 0): ?>
									<span class="message-unread"><?php printf(__('%d unread', 'ab'), $unread_count); ?></span>
								<?php endif; ?>
								<span class="message-date"><span class="from" title="<?php _e( 'Sent message', 'ab' ); ?>"><i class="i-mail-reply"></i></span><?php bp_message_thread_last_post_date(); ?></span>
							</td>
						<?php endif; ?>
